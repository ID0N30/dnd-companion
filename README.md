...
git branch -M main
git remote add origin https://github.com/tu-usuario/dnd-companion.git
git push -u origin main
```

### Paso 2: Importar en Vercel
1. Ingresa a [Vercel Dashboard](https://vercel.com/dashboard) y haz clic en **"Add New..." > "Project"**.
2. Selecciona tu repositorio de GitHub `dnd-companion`.
3. En la sección **Environment Variables**, agrega cada una de las variables `NEXT_PUBLIC_FIREBASE_*` configuradas en tu `.env.local`.
4. Presiona **Deploy**.

### Paso 3: Autorizar Dominio en Firebase Console
1. Ve a [Firebase Console](https://console.firebase.google.com/) > tu proyecto > **Authentication** > pestaña **Settings** > **Authorized domains**.
2. Agrega la URL asignada por Vercel (ejemplo: `dnd-companion.vercel.app`).

---

## 🛡️ Reglas de Seguridad de Firestore (`firestore.rules`)

Para producción, asegúrate de publicar las siguientes reglas en **Firebase Console > Firestore Database > Reglas**:
...
Este proyecto está distribuido bajo la licencia MIT. ¡Siéntete libre de adaptarlo para tus partidas de rol! 🎲🧙‍♂️🐉
